...
class Config:
    def __init__(self):
        ...
        # 앙상블 다양성을 위해 각 모델마다 다른 시퀀스 길이 사용
        self.SEQUENCE_LENGTHS = [30, 40, 50, 60, 70]
        # 각 모델마다 다른 random_state 사용
        self.RANDOM_STATES = [7, 42, 123, 2024, 777]
        # 각 모델마다 다른 dropout 비율 사용
        self.DROPOUT_RATES = [0.3, 0.35, 0.4, 0.45, 0.5]
    ...
    try:
        result_data = {
            ...
            "config": {
                ...
                "MODEL_ACCURACY_AVG": acc_ave,
                "MODEL_LOSS_AVG" : loss_ave,
                "MODEL_ACCURACIES": config.MODEL_ACCURACY,
                "MODEL_LOSSES": config.MODEL_LOSS,
            },
...
def predict_next(model, latest_data):
    ...
    # 2. 예측 숫자 가공 바이너리벡터[0,1,0,0,0,1,0,0...]의 인덱스=>[0,1,2,3,4,5 ...]에서, 0번째 인덱스를 삭제
...
def train_model(model, epochs, patience, X_train, X_test, Y_train, Y_test):
    ...
    history = model.fit(
        ...
        verbose=0,
        callbacks=[early_stopping, minimal_logger]
    )
...
def create_model(input_shape, dropout_rate):
    model = Sequential([
        ...
        # 첫 번째 LSTM 레이어 - BatchNormalization 추가
        ...
        Dropout(dropout_rate * 0.5),  # 출력층에 가까울수록 낮은 dropout

        # 출력층: 46개의 출력 (0~45번 숫자)
...
def main():
    ...
    for i in range(config.ENSEMBLE_COUNT):
        ...
        logger.info(f"  - RANDOM_STATE: {config.RANDOM_STATES[i]}")
        print(f"\n")
        ...
        # 각 모델마다 다른 random_state 사용
        random_state = config.RANDOM_STATES[i]
